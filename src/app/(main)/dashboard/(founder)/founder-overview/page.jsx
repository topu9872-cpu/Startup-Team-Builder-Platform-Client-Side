  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#0f111a] border border-slate-800 p-3 rounded-xl shadow-2xl backdrop-blur-md">
        <p className="text-[10px] font-bold tracking-wider text-slate-400 uppercase">{data.name}</p>
        <p className="text-lg font-bold text-white mt-0.5">{payload[0].value}</p>
      </div>
    );
  }
  return null;
...
export default function FounderOverviewPage() {
  ...
  const chartData = [
    { name: "Opportunities", value: stats.totalOpportunities, colorStart: "#a855f7", colorEnd: "#7c3aed" },
    { name: "Applications", value: stats.totalApplications, colorStart: "#3b82f6", colorEnd: "#2563eb" },
    { name: "Accepted", value: stats.acceptedMembers, colorStart: "#10b981", colorEnd: "#059669" },
  ];
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800/60 pb-5">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">Founder Hub Overview</h1>
          <p className="text-xs text-slate-400 mt-0.5">Track venture pipelines, conversion ratios, and developer ecosystem growth nodes.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-xl bg-[#0d0e12] border border-slate-800/80 shadow-lg relative group transition-all duration-300 hover:border-purple-500/30">
          ...
          <div className="mt-4">
            ...
            <p className="text-xs font-medium text-slate-400 mt-0.5">Total Core Opportunities</p>
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl bg-[#0d0e12] border border-slate-800/80 shadow-lg relative group transition-all duration-300 hover:border-blue-500/30">
          ...
          <div className="mt-4">
            <h2 className="text-2xl font-bold tracking-tight text-white">{stats.totalApplications}</h2>
            <p className="text-xs font-medium text-slate-400 mt-0.5">Incoming Applications</p>
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl bg-[#0d0e12] border border-slate-800/80 shadow-lg relative group transition-all duration-300 hover:border-emerald-500/30">
          ...
          <div className="mt-4">
            <h2 className="text-2xl font-bold tracking-tight text-white">{stats.acceptedMembers}</h2>
            <p className="text-xs font-medium text-slate-400 mt-0.5">Accepted Contributors</p>
          </div>
        </div>
        ...
        <div className="flex items-start justify-between border-b border-slate-800/60 pb-4 mb-6">
          <div className="space-y-0.5">
            <h2 className="text-sm font-bold tracking-tight text-white flex items-center gap-2">
              <TrendingUp className="size-4 text-purple-400" /> Pipeline Conversions
            </h2>
            <p className="text-xs text-slate-400">Visual mapping comparing opportunity generation to closed applications.</p>
          </div>
        </div>
        ...
        <div className="h-75 w-full text-xs">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
              <defs>
                {chartData.map((data) => (
                  <linearGradient key={data.name} id={`grad-${data.name}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={data.colorStart} stopOpacity={1} />
                    <stop offset="100%" stopColor={data.colorEnd} stopOpacity={0.15} />
                  </linearGradient>
                ))}
              </defs>
              ...
              <Bar dataKey="value" radius={[6, 6, 0, 0]} maxBarSize={50}>
                {chartData.map((entry) => (
                  <Cell key={`cell-${entry.name}`} fill={`url(#grad-${entry.name})`} stroke={entry.colorStart} strokeWidth={1} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
