...
const ModalFieldInput = ({ label, name, value, onChange, placeholder, type = "text", isSelect = false, options = [], rows = 3 }) => {
  const inputClass = "w-full h-11 px-3.5 rounded-lg bg-[#1a1d26] border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/20 transition-all text-sm";
  const labelClass = "block mb-1 text-[10px] font-semibold uppercase tracking-wider text-slate-400";
    ...
    <div>
      ...
      {isSelect ? (
        <select name={name} value={value} onChange={onChange} className={`${inputClass} cursor-pointer`}>
          ...
          {options.map(opt => <option key={opt} value={opt} className="bg-[#1a1d26]">{opt}</option>)}
        </select>
      ) : type === "textarea" ? (
        <textarea name={name} rows={rows} value={value} onChange={onChange} placeholder={placeholder} className={`${inputClass} h-auto p-4 resize-none`} />
      ) : (
        <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className={`${inputClass} ${type === 'date' ? 'scheme-dark' : ''}`} />
      )}
    </div>
  );
};
...
export default function UpdateOpportunityModal({ initialData }) {
  ...
  const technicalFields = [
    { name: "roleTitle", label: "Role Title", placeholder: "e.g., Lead Infrastructure Engineer" },
    { name: "startupName", label: "Startup Name", placeholder: "e.g., SkyForge Systems" },
    { name: "requiredSkills", label: "Required Skills Matrix", placeholder: "AWS, Terraform, Go, Kubernetes", spansFull: true },
    { name: "sector", label: "Sector", placeholder: "e.g., Deep Tech / Cloud" },
    { name: "corePulseBackendSystems", label: "CorePulse Backend Engine", placeholder: "Distributed Pipelines, Kafka" },
    { name: "ecosystemSegment", label: "Ecosystem Segment", placeholder: "e.g., B2B Infrastructure SaaS" }
  ];

  const logisticFields = [
    { name: "workType", label: "Work Structure", placeholder: "Select Structure", isSelect: true, options: ["Remote", "Hybrid", "Onsite"] },
    { name: "workEnvironmentAllocation", label: "Environment Allocation Ratio", placeholder: "e.g., 80% Remote / 20% Onsite" },
    { name: "commitmentLevel", label: "Commitment Level", placeholder: "e.g., Full-Time Core Contributor" },
    { name: "compensation", label: "Compensation Structure", placeholder: "e.g., $160k - $210k + 1.5% Equity" },
    { name: "applicationDeadline", label: "Application Close Window", type: "date" }
  ];
    ...
    <Modal>
      ...
      <Modal.Backdrop className="bg-black/70 backdrop-blur-xs transition-all">
        <Modal.Container placement="center" className="p-4">
          <Modal.Dialog className="w-full sm:max-w-3xl bg-[#0d0e12] border border-slate-800/80 rounded-2xl overflow-hidden shadow-2xl relative text-sm text-slate-100">
            ...
            <Modal.Header className="p-6 pb-4 border-b border-slate-800/60 bg-linear-to-b from-slate-900/20 to-transparent">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Modal.Heading className="text-lg font-bold tracking-tight text-white">Update Pipeline Opportunity</Modal.Heading>
                  <p className="text-xs text-slate-400 mt-0.5">Modify functional parameters, resource allocation blocks, and description blueprints.</p>
                </div>
              </div>
            </Modal.Header>
            ...
            <Modal.Body className="p-5 sm:p-6 max-h-[75vh] overflow-y-auto space-y-5 custom-scrollbar">
              <Surface className="bg-transparent border-0 p-0 shadow-none">
                <form id="update-opportunity-form" onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="bg-[#161820]/40 p-4 rounded-xl border border-slate-800/60 space-y-3.5">
                    <h4 className="text-[10px] font-bold text-purple-400 tracking-wider uppercase flex items-center gap-1.5">
                      <Briefcase className="size-3 text-purple-400" /> Engineering Specs & Architectural Core
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      {technicalFields.map(field => (
                        <div key={field.name} className={field.spansFull ? "sm:col-span-2" : ""}>
                          <ModalFieldInput {...field} value={formData[field.name]} onChange={handleChange} />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-[#161820]/20 p-4 rounded-xl border border-slate-800/40 space-y-3.5">
                    <h4 className="text-[10px] font-bold text-indigo-400 tracking-wider uppercase flex items-center gap-1.5">
                      <Calendar className="size-3 text-indigo-400" /> Terms, Allocation & Deployment Metrics
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      {logisticFields.map(field => (
                        <div key={field.name} className={field.name === "applicationDeadline" ? "sm:col-span-2" : ""}>
                          <ModalFieldInput {...field} value={formData[field.name]} onChange={handleChange} />
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-1">
                    <h4 className="text-[10px] font-bold text-slate-400 tracking-wider uppercase flex items-center gap-1.5 mb-2">
                      <FileText className="size-3 text-slate-500" /> Operational Overview Narrative
                    </h4>
                    <ModalFieldInput 
                      name="description" 
          ...
          </Modal.Dialog> {/* Fixed Tag Line Here */}
        </Modal.Container>
      </Modal.Backdrop>
    </Modal>
  );
