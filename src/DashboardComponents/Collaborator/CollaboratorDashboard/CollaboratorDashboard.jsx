...
export default function CollaboratorDashboard() {
  ...
  const chartData = [
    { name: "Total Applications", value: stats.totalApplications, fill: "#3b82f6" },
    { name: "Accepted Roles", value: stats.accepted, fill: "#10b981" },
    { name: "Rejected Node Tracks", value: stats.rejected, fill: "#ef4444" },
  ];

  // Calculate success conversion against closed loops
  const totalDecided = stats.accepted + stats.rejected;
  const successRate = totalDecided > 0 ? ((stats.accepted / totalDecided) * 100).toFixed(0) : 0;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800/60 pb-5">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">Collaborator Space</h1>
          <p className="text-xs text-slate-400 mt-0.5">Track your submission pipelines, processing velocity, and historical outcome states.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-xl bg-[#0d0e12] border border-slate-800/80 shadow-lg transition-all duration-300 hover:border-blue-500/30">
          ...
          <div className="mt-4">
            ...
            <p className="text-xs font-medium text-slate-400 mt-0.5">Total Applications</p>
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl bg-[#0d0e12] border border-slate-800/80 shadow-lg transition-all duration-300 hover:border-emerald-500/30">
          ...
          <div className="mt-4">
            ...
            <p className="text-xs font-medium text-slate-400 mt-0.5">Accepted Roles</p>
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl bg-[#0d0e12] border border-slate-800/80 shadow-lg transition-all duration-300 hover:border-red-500/30">
          ...
          <div className="mt-4">
            ...
            <p className="text-xs font-medium text-slate-400 mt-0.5">Total Rejected</p>
          </div>
        </div>
      ...
      <div className="bg-[#0d0e12] border border-slate-800/80 p-5 sm:p-6 rounded-xl shadow-xl grid grid-cols-1 md:grid-cols-5 gap-6 items-center">
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="space-y-1">
            <h2 className="text-sm font-bold tracking-tight text-white flex items-center gap-2">
              <TrendingUp className="size-4 text-blue-400" /> Lifecycle Status Ratios
            </h2>
            <p className="text-xs text-slate-400 leading-relaxed">
              Monitoring application vectors. The dynamic ratio maps successful onboardings directly against your total closed recruitment loops.
            </p>
          </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-900">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-white font-semibold ml-auto">{stats.totalApplications}</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <div className="size-2 rounded-full bg-emerald-500" />
              <span className="text-slate-400">Accepted Roles</span>
              <span className="text-white font-semibold ml-auto">{stats.accepted}</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <div className="size-2 rounded-full bg-red-500" />
              <span className="text-slate-400">Archived/Rejected</span>
              <span className="text-white font-semibold ml-auto">{stats.rejected}</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-3 h-65 w-full relative flex items-center justify-center">
          ...
          <div className="absolute flex flex-col items-center justify-center pointer-events-none">
            <span className="text-2xl font-black text-white tracking-tight">{successRate}%</span>
            <span className="text-[9px] uppercase font-bold tracking-widest text-slate-500 mt-0.5">Win Rate</span>
          </div>
        </div>
